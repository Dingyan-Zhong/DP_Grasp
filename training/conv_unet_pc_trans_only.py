...
from DP_Grasp.data.grasp_dataset import PC_R9_Trans_Only_Dataset
...
def train_epoch(
    ...
    for batch in train_loader:
        ...
        top_grasp = batch['top_grasp_r9'].to(device)
        ...
        with torch.no_grad():
            ob_center = batch['obj_center'].to(device)
            ob_max_dist = batch['obj_max_dist'].to(device)
            top_grasp_r9 = batch['top_grasp_r9'].to(device)
            ...
            top_pos_norm_pred_dist = torch.norm(top_pos_normalized - pred_pos_normalized, dim=1)
            top_pos_norm_pred_dist_loss = nn.functional.mse_loss(top_pos_norm_pred_dist, torch.zeros_like(top_pos_norm_pred_dist))
            

        epoch_loss.append(loss.item())
        epoch_pos_loss.append(top_pos_pred_dist_loss.item())
        epoch_norm_pos_loss.append(top_pos_norm_pred_dist_loss.item())

    return np.mean(epoch_loss), np.mean(epoch_pos_loss), np.mean(epoch_norm_pos_loss)
...
def main(config: ConvUnetTrainingConfig):
    ...
    train_dataset = PC_R9_Trans_Only_Dataset(config.dataset_path, split="train")
    val_dataset = PC_R9_Trans_Only_Dataset(config.dataset_path, split="val")
    ...
    noise_pred_net = ConditionalUnet1D(input_dim=9, global_cond_dim=512).to(device)
    ...
    with tqdm(range(config.epochs), desc='Epoch') as tglobal:
        for epoch in tglobal:
            model.train()
            train_loss, train_pos_loss, train_norm_pos_loss = train_epoch(model, 
                                     ema_model, 
                                     optimizer, 
                                     lr_scheduler, 
                                     noise_scheduler, 
            ...
            model.eval()
            with torch.no_grad():
                val_loss, val_pos_loss, val_norm_pos_loss = train_epoch(model, 
                                       ema_model, 
                                       optimizer, 
                                       lr_scheduler, 
                                       noise_scheduler, 
            ...
            if config.use_wandb:
                wandb.log({
                    ...
                    "val_loss": val_loss,
                    "train_pos_mse": train_pos_loss,
                    "val_pos_mse": val_pos_loss,
                    "train_normalized_pos_mse": train_norm_pos_loss,
                    "val_normalized_pos_mse": val_norm_pos_loss
                })
